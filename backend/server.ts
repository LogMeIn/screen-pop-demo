...
const notificationChannelApi: GoToApiService = new NotificationChannelApi(
  new ChannelRequest(),
);
const screenPopEndpoint = new ScreenPopEndpoint(notificationChannelApi);

// Add cors support
app.use(cors(Util.corsOptions));
app.use((req, res, next) => Util.setupLogger(logger, req, res, next));

const httpServer = http.createServer(app);

httpServer.listen(Util.port, () =>
  logger.info(`Server is running at http://${Util.hostname}:${Util.port}`),
);
